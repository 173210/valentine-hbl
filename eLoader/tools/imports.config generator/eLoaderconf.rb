...
nb_nids = 0;
nids_offset = 12 + stub_addresses.size * 4
config.each {|libinfo | 
...
out_conf = File.new( root + "config/imports.config", "w");

out_conf.write(stub_addresses.size.mips(4));
out_conf.write(config.size.mips(4));
out_conf.write(nb_nids.mips(4));
stub_addresses.each{ |stub_addr|
