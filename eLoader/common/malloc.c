...
{
	SceUID blockid;
    	dbg_printf("-->ALLOCATING MEMORY from partition 2, size 0x%08X... ", size);

	int i = find_free_block();
	if(i == MAX_ALLOCS) {
		dbg_printf("WARNING: no free blocks remaining\n");
		return NULL;
	}

	dbg_printf("Found free block %d\n", i);
	...
	if(blockid < 0) {
		dbg_printf("WARNING: malloc failed with error 0x%08X\n", blockid);
		return NULL;
	}

	dbg_printf("Got block from kernel with UID 0x%08X\n", blockid);
