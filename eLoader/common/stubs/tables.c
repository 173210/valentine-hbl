...
u32 get_klowest_syscall(tSceLibrary* library, int ref_lib_index, int is_cfw)
{
	...
	dbg_printf("Get lowest syscall for %s\n", (u32)library->name);
	...
	if (globals->syscalls_known)
	{
		if (globals->module_sdk_version <= 0x06010010)
		{
			...
			dbg_printf("Lowest syscall is %d\n", lowest_syscall);
		}
#ifdef XMB_LAUNCHER
		...
		else
		{
				...
				if (kdump_size > 0)
				{
					...
					if (lowest_offset > 0)
					{
						...
						else
						{
							...
							library->gap = 0;
							dbg_printf("->library doesn't wrap, gap set to 0\n");
						}
					}
				}
				else
				{
					dbg_printf("Kmemdump empty\n");
				}
			...
			{
				dbg_printf("Could not open kernel dump\n");
			}
		}
	}
...
tSceLibrary* complete_library(tSceLibrary* plibrary, int UNUSED(ref_lib_index), int UNUSED(is_cfw))
{
	...
	if (nid_file > -1)
	{
		...
		NID_DBG_PRINTF("Getting lowest syscall\n");
		...
		if ((lowest_syscall & SYSCALL_MASK_RESOLVE) == 0)
		{
			...
			if (syscall_gap < 0)
			{
				...
				dbg_printf("ERROR: lowest syscall in kernel (0x%08X) is bigger than found lowest syscall in tables (0x%08X)\n", lowest_syscall,
				        plibrary->lowest_syscall);
			}

			else if (syscall_gap == 0)
			{
				NID_DBG_PRINTF("Nothing to do, lowest syscall from kernel is the same from tables\n");
			}
...
{
    	dbg_printf("Searching for library %s\n", lib_name);
	if (lib_name == NULL)
        return -1;
...
{
    	NID_DBG_PRINTF("Adding NID 0x%08X to table... ", (int)nid);
...
int add_stub_to_table(tStubEntry *pentry)
{
	...
	NID_DBG_PRINTF("-->Processing library: %s ", (u32)(pentry->lib_name));
	...
	if (!(good_call & SYSCALL_MASK_RESOLVE) &&
		...
		for(nid_index = 0; nid_index < pentry->stub_size; nid_index++) {
			nid = *cur_nid;
			NID_DBG_PRINTF("--Current NID: 0x%08X", nid);
...
void dump_lib_table()
{
	...
	if (globals->syscalls_known && (globals->module_sdk_version <= 0x06010010))
	{
		...
		for (lib_index = 0; lib_index < globals->lib_table.num; lib_index++) {
			...
			dbg_printf("0x%08X %s %s %d ", globals->module_sdk_version,
				...
				(int)num_lib_exports);
			dbg_printf("%d ", globals->lib_table.table[lib_index].highest_syscall - globals->lib_table.table[lib_index].lowest_syscall + 1);
	...
	for (lib_index = 0; lib_index < globals->lib_table.num; lib_index++) {
		...
		for (nid_index = 0; nid_index < globals->nid_table.num; nid_index++) {
			if (globals->nid_table.table[nid_index].lib_index == lib_index) {
				...
				dbg_printf("[%d] 0x%08X 0x%08X ", nid_index, globals->nid_table.table[nid_index].nid, syscall);
